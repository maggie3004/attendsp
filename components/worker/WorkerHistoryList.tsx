...
export function WorkerHistoryList({ records }: { records: Record[] }) {
  if (records.length === 0) {
    return (
      <div className="card py-12 flex flex-col items-center gap-3 text-foreground/40">
      ...
      {records.map((record) => (
        <div key={record.id} className="card p-4 flex items-center gap-4">
          ...
          <div className="text-center flex-shrink-0 w-12">
            <div className="text-lg font-bold leading-none">{formatDate(record.date, 'd')}</div>
            <div className="text-xs text-foreground/40 mt-0.5">{formatDate(record.date, 'EEE')}</div>
          </div>
