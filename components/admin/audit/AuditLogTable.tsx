...
export function AuditLogTable({ logs }: { logs: AuditLog[] }) {
  return (
    <div className="card overflow-hidden">
      {logs.length === 0 ? (
        <div className="py-12 text-center text-foreground/40 text-sm">No audit logs yet</div>
      ) : (
              ...
              {logs.map((log) => (
                <tr key={log.id} className="hover:bg-surface-elevated/50 transition-colors">
                  <td className="px-5 py-3.5 whitespace-nowrap">
                    ...
                    <div className="text-[10px] text-foreground/30">{formatDateTime(log.createdAt)}</div>
                  </td>
                  <td className="px-5 py-3.5">
                    {log.actor ? (
                      <>
                        <div className="font-medium">{log.actor.name}</div>
                        <div className="text-xs text-foreground/40">{log.actor.employeeId}</div>
                      </>
                    ) : <span className="text-foreground/30">System</span>}
                  </td>
                  <td className="px-5 py-3.5">
                    <span className={cn('px-2.5 py-1 rounded-full text-xs font-medium border', ACTION_COLORS[log.action] ?? 'text-foreground/60 bg-surface-elevated border-surface-border')}>
                      {log.action.replace(/_/g, ' ')}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-foreground/60">
                    <div className="text-xs">{log.targetTable}</div>
                    {log.targetUser && <div className="text-xs text-foreground/40">{log.targetUser.name}</div>}
                  </td>
                  <td className="px-5 py-3.5 text-foreground/60 max-w-48 truncate">{log.reason ?? '—'}</td>
                  <td className="px-5 py-3.5 text-foreground/30 text-xs font-mono">{log.ipAddress ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
